...
class EqualizerScreen extends StatelessWidget {
                ...
                child: Column(
                  children: [
                    ...
                    _buildPresetsSection(
                        controller, screenWidth, isSmallScreen),

                    SizedBox(height: screenHeight * 0.03),

                    // Equalizer Section
                    _buildEqualizerSection(
                        controller, screenWidth, screenHeight, isSmallScreen),
  ...
  void _showStatusFeedback(
      BuildContext context, EqualizerController controller) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor:
            controller.isEqualizerEnabled.value ? Colors.green : Colors.grey,
  ...
  Widget _buildPresetsSection(
      EqualizerController controller, double screenWidth, bool isSmallScreen) {
    return Container(
  ...
  Widget _buildEqualizerSection(EqualizerController controller,
      ...
      child: Column(
        children: [
          ...
          SizedBox(
            ...
            child: Obx(() =>
                _buildEqualizerBands(controller, screenWidth, isSmallScreen)),
          ),
        ],
      ),
    );
  ...
  Widget _buildCompactEqualizerBands(
      EqualizerController controller, double screenWidth, bool isSmallScreen) {
    final availableWidth =
        screenWidth - 120; // Much more padding to prevent overflow
    final bandWidth =
        availableWidth / EqualizerController.frequencyBands.length;
  ...
  Widget _buildScrollableEqualizerBands(
      EqualizerController controller, double screenWidth, bool isSmallScreen) {
          ...
          children: List.generate(
            ...
            (index) => Padding(
              padding: EdgeInsets.only(
                left: index == 0 ? 0 : 6,
                right: index == EqualizerController.frequencyBands.length - 1
                    ? 0
                    : 6,
              ),
  ...
  Widget _buildEqualizerBands(
    ...
    final bandWidth =
        availableWidth / EqualizerController.frequencyBands.length;
    final sliderWidth =
        bandWidth.clamp(15.0, 20.0); // Much smaller width to fit all bands
              ...
              child: SliderTheme(
                data: SliderTheme.of(Get.context!).copyWith(
                  ...
                  thumbShape: RoundSliderThumbShape(
                      enabledThumbRadius: isSmallScreen ? 2 : 3),
                  trackHeight: isSmallScreen ? 1 : 1.5,
                  overlayShape: RoundSliderOverlayShape(
                      overlayRadius: isSmallScreen ? 4 : 6),
              ...
              child: RotatedBox(
                ...
                child: SliderTheme(
                  data: SliderTheme.of(Get.context!).copyWith(
                    ...
                    overlayShape: RoundSliderOverlayShape(
                        overlayRadius: isSmallScreen ? 12 : 16),
                    overlayColor: _getBandColor(gain).withOpacity(0.2),
                    trackShape: const RoundedRectSliderTrackShape(),
                  ),
                  child: Slider(
  ...
  Color _getBandColor(double gain) {
    if (gain > 0) {
      ...
      return Color.lerp(Colors.green, Colors.yellow, gain / 12.0) ??
          Colors.green;
    } else if (gain < 0) {
      // Negative gain - blue to purple
      return Color.lerp(Colors.blue, Colors.purple, gain.abs() / 12.0) ??
          Colors.blue;
    } else {
      // Flat - white
      return Colors.white;
